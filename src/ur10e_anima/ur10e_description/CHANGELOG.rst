^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Changelog for package ur10e_description
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
* ur10e_description: Make robot name configurable on load_ur.launch (`#612 <https://github.com/ros-industrial/universal_robot/issues/612>`_)
...
* fixing deprecation warning
* ur10e_description: Use the per-model xacro files for correct robot name (`#588 <https://github.com/ros-industrial/universal_robot/issues/588>`_)
...
* Update ur10e_description/urdf/ur_macro.xacro
...
* removed --inorder for common launch
  * default in melodic
* update ur10e_description launch files
...
* made common macro for ur_robot urdf
  ...
  * remved ur10e_gazebo specific parts
* create parameters yaml files
  * one for each models
* deleted ur_e specific repositories
  * moved ur_e_description meshes files to ur10e_description
